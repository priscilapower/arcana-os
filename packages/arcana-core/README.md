...
On top of subscriptions, an agent can carry **guardrails** — declarative rules (`DENY_TOOL`, `SCOPE_PATHS`, `MAX_FILE_SIZE`, `REQUIRE_CONFIRMATION`) resolved once per run from the World's system rules and the agent's own, then evaluated *before* routing, so a blocked write or delete never reaches the adapter. `SCOPE_PATHS` is applied to **every** path argument a tool declares, so `move` and `copy` are checked on their destination as well as their source. Card archetypes seed the rules: The Hermit is denied every mutating tool. See the [Tools API reference](https://docs.arcanaos.cloud/api/tools/) for the full contract.
